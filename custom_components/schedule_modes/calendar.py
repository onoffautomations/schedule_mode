...
class ModeCalendar(CalendarEntity):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry, store: Store, events_all: List[Dict[str, Any]], mode_key: str, custom_name: Optional[str] = None):
        self.hass = hass
        self.entry = entry
        self.store = store
        self.mode_key = mode_key
        self.is_custom = custom_name is not None
        # Use the custom name if provided, otherwise use mode_friendly
